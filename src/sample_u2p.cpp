...
double log_u2p_pdf(double x, void* params){
  ...
  int wave_num = d->it_inter.n_rows;
  double tmp = d->it_inter(0) + d->d_tpfp * x;
  ...
  for(unsigned int t=1; t<wave_num; ++t){
    tmp = d->it_inter(t) + d->beta_tp_fp(0) * d->Ci_tpfp(t-1) + d->beta_tp_fp(1) * d->Ci_fptp(t-1) + x;
...
arma::vec sample_u2p_cpp(arma::mat it_inter, arma::mat xcovs, double d_tpfp,
  ...
  for(unsigned int i=0;i<N;++i){
    ...
    log_u2p_data.it_inter = it_inter.row(i).t();
