...
#' @export latent_trans
latent_trans <- function(y_tp, C_it_tp, u_tp0, alpha_tp0, struc_params0, mcmc_len){
  bj <- struc_params0[1:wave_num]
  d1 <- struc_params0[wave_num+1]
  beta <- struc_params0[wave_num+2]
  sigma2 <- struc_params0[wave_num+3]
  ...
  for(iter in 1:mcmc_len){
    ...
    # parts1 <- parts1 + 1/(1+exp(-tmp)) + C_it_tp[N+1:N] * tmp1 - log(1+exp(tmp1))
    # lambda0 <- t(alpha_tp0[1,] + outer(alpha_tp0[2,], rep(0,N)))
    # parts0 <- rowSums(lambda0 * y_tp[1:N,] - log(1+exp(lambda0)))
    # tmp0 <- bj[2] + u_tp0
    # parts0 <- parts0 + 1/(1+exp(tmp)) + C_it_tp[N+1:N] * tmp0 - log(1+exp(tmp0))
    ...
    # parts1 <- parts1 + 1 / (1+exp(-tmp)) + C_it_tp[2*N+1:N] * tmp1 - log(1+exp(tmp1))
    # parts0 <- rowSums(lambda0 * y_tp[N+1:N,] - log(1+exp(lambda0)))
    # tmp0 <- bj[3] + u_tp0
    # parts0 <- parts0 + 1 / (1+exp(tmp)) + C_it_tp[2*N+1:N] * tmp0 - log(1+exp(tmp0))
    ...
    # parts1 <- parts1 + 1 / (1+exp(-tmp))
    # parts0 <- rowSums(lambda0 * y_tp[N+1:N,] - log(1+exp(lambda0)))
    # parts0 <- parts0 + 1 / (1+exp(tmp))
